...
## Power BI 打不开时优先检查（已修复）
- 若出现 `Error converting value "File" to type PbidsMode`：
  - 检查 `R2R_Local_Dataset.pbids` 中 `mode` 是否为 `Import`（不能是 `File`）。

## 真实可视化结果文件（N09 输出）
- `D:\R2R_Automation\pbi\powerbi_visual_preview.html`
- `D:\R2R_Automation\pbi\chart_revenue_trend.png`
- `D:\R2R_Automation\pbi\kpi_latest_snapshot.csv`
## 可视化升级结果（已落地）
- 营运资本效率：`D:\R2R_Automation\pbi\chart_wc_efficiency.png`
- AR账龄堆叠柱：`D:\R2R_Automation\pbi\chart_ar_aging_stacked.png`
- 现金流瀑布图：`D:\R2R_Automation\pbi\chart_cashflow_waterfall.png`
- FCF与利润含金量：`D:\R2R_Automation\pbi\chart_fcf_profit_quality.png`
- 可视化预览页：`D:\R2R_Automation\pbi\powerbi_visual_preview.html`

## 预览格式规范
- 报表网页预览中：`环比%`、`同比%`（及兼容列 `revenue_mom_pct`/`mom_pct`/`yoy_pct`）统一四位小数。
- KPI 快照表在网页中采用精简字段与横向自适应，避免右侧溢出。
- 可视化趋势区标题统一为“营收趋势分析图”。
